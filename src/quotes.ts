...
interface FetchQuoteError extends Error {
  status?: number;
}

// Fetch a random quote within the specified length range
export const fetchQuote = async (): Promise<QuoteData | null> => {
  try {
    ...
    if (!response.ok) {
      const error: FetchQuoteError = new Error(`HTTP error! status: ${response.status}`);
